...
function solve_laplace_eq_on_sphere(refinement, u_exact::Function, f_rhs::Function)
    Ωₕ = create_embedded_sphere(refinement)
    ...
    u_ex = apply_analytical!(u_ex, dh, :u, u_exact(x[1], x[2], x[3]))
    err = u_h .- u_ex
    println("nnodes            : ", n)
